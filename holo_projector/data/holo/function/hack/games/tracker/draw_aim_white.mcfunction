# holo:hack/games/tracker/draw_aim_white — draw 3x3 white crosshair at (#TR_AIM_R, #TR_AIM_C)
...
execute as @e[tag=projector_pixel] if score @s holo.row >= #TR_AR_M holo.tmp if score @s holo.row <= #TR_AR_P holo.tmp if score @s holo.col >= #TR_AC_M holo.tmp if score @s holo.col <= #TR_AC_P holo.tmp run data modify entity @s background set value -1
