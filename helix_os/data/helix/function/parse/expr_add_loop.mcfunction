# helix:parse/expr_add_loop — Loop consuming + or - operators
...
scoreboard players set helix _is_add_op 0
execute if data storage helix:vm _cur_tok{t:"OP",v:"+"} run scoreboard players set helix _is_add_op 1
execute if data storage helix:vm _cur_tok{t:"OP",v:"-"} run scoreboard players set helix _is_add_op 1
execute unless score helix _is_add_op matches 1 run return 0
...
return run function helix:parse/expr_add_loop
